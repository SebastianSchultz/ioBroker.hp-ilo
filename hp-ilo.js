...
adapter.on('stateChange', function (id, state)
{
    if (id && state && !state.ack)
	{
		...
		switch (id)
		{
			...
			case 'channelId':
				adapter.log.debug('Sending switch to channelId ' + state.val + ' command to WebOS TV: ' + adapter.config.ip);
				sendCommand('ssap://tv/openChannel', {channelId: state.val}, function (err, val) {
					if (!err) 
						adapter.setState('channelId', state.val, true)
					else 
						adapter.log.debug('Error in switching to channelId: ' + err);
				});
				break;
